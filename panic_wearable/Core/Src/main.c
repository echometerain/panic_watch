...
/* USER CODE BEGIN PV */
static bool prev_button_on = true;
bool panic_mode = false;

static Lcd_PortType ports[] = { GPIOC, GPIOC, GPIOC, GPIOC, GPIOC, GPIOC,
GPIOC,
GPIOC };
static Lcd_PinType pins[] = { GPIO_PIN_0, GPIO_PIN_1, GPIO_PIN_2,
GPIO_PIN_3,
...
extern volatile bool user_is_panicking;
/* USER CODE END PV */
...
int _write(int file, char *ptr, int len) { // override default printf
	int DataIdx;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) { // override
	ADC_complete = true;
}

static void lcd_init() {
	lcd = Lcd_create(ports, pins, GPIOC, GPIO_PIN_8, GPIOD, GPIO_PIN_2,
			LCD_8_BIT_MODE);
	HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);
}

static void check_button() { // check button state
	bool new_button_on = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);
	if (prev_button_on == false && new_button_on == true) {
		if (panic_mode) {
			unpanic();
		} else {
			panic();
...
void handle_msg_it() { // handle motivational message interrupt
	...
	for (int i = 0; i < NUM_LINES; i++) {
		...
		if (end_ch_idx < msg_strlen[msg_idx]) {
			char *to_null = &msg[msg_idx][end_ch_idx];
			char ch = *to_null;
			*to_null = '\0';
			Lcd_string(&lcd, &msg[msg_idx][(msg_line + i) * CHAR_PER_LINE]);
			*to_null = ch;
		} else {
			Lcd_string(&lcd, &msg[msg_idx][(msg_line + i) * CHAR_PER_LINE]);
			msg_finished = true;
...
// DMA MUST BE INIT'd BEFORE ADC!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  /* USER CODE BEGIN 2 */
	...
	for (int i = 0; i < NUM_MSG; i++) {
		msg_strlen[i] = strlen(msg[i]);
	}

// set random seed
	srand(0); // can't use system time :(

// test if print is working
	printf(" print!\n"); // first space doesn't print
	fflush(stdout);
